...
function negetgan(){
  Cookies=$(curl -s -I "http://sms.payuterus.biz/alpha/send.php" -H "Mozilla/5.0 (Linux; Android 4.4.2; Nexus 4 Build/KOT49H) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/34.0.1847.114 Mobile Safari/537.36"|grep -Po '(?<=PHPSESSID=)[^,]*'|cut -d ';' -f1)
         bypass=$(curl -s 'http://sms.payuterus.biz/alpha/index.php?a=keluar' \
           -H 'Connection: keep-alive' \
           -H 'Cache-Control: no-cache' \
           -H 'Upgrade-Insecure-Requests: 1' \
           -H "Mozilla/5.0 (Linux; Android 4.4.2; Nexus 4 Build/KOT49H) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/34.0.1847.114 Mobile Safari/537.36" \
           -H 'Referer: http://sms.payuterus.biz/alpha/send.php' \
           -H "Cookie: PHPSESSID=$Cookies;" --compressed --insecure | grep "value" | grep "name" | grep -oP 'value="\K[^"]+')
          okeey1=$(curl -s 'http://sms.payuterus.biz/alpha/index.php?a=keluar' \
           ...
           -H "Cookie: PHPSESSID=$Cookies;" --compressed --insecure | grep "span" | grep -Po "\d \S \d")
         if [[ $okeey1 =~ "+" ]]; then
           ok=`expr $okeey1 | bc`
         elif [[ $okeey =~ "-" ]]; then
          ok=`expr $okeey1 | bc`
         ...
         -H "Cookie: PHPSESSID=$Cookies;" --data "nohp=$now&pesan=$psn&captcha=$ok&key=$bypass" --compressed --insecure | grep -o 'value="[^"]*"')
